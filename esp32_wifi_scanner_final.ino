/*
 * Scanner WiFi ESP32 v2.1 - Version Finale
 * 
 * Fonctionnalités :
...
// 🔊 Fonctions pour les alertes sonores
void playStartupSound() {
  // Mélodie de démarrage : 3 bips courts
  for (int i = 0; i < 3; i++) {
  ...
  Serial.println("🔊 Buzzer initialisé - Test sonore OK");
}

void playNewOpenNetworkAlert(String ssid) {
  Serial.printf("🔊 ALERTE SONORE: Nouveau réseau ouvert détecté! %s\n", ssid.c_str());
  
  // Séquence sonore spéciale pour réseau ouvert
  // 2 bips longs + 3 bips courts
  for (int i = 0; i < 2; i++) {
    digitalWrite(BUZZER_PIN, HIGH);
    delay(300);
    digitalWrite(BUZZER_PIN, LOW);
